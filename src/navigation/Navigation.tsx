...
import { GitMan } from '../games/GitMan'
...
export function Navigation (): JSX.Element {
  ...
  const [showGitMan, setShowGitMan] = React.useState(false)
  ...
  function resetAnimations (resultLength: string): void {
  ...
  const navigate = (form: FormEvent<HTMLFormElement>) => {
    ...
    setHistory1({ command, result })
    resetAnimations(length * 0.012 + 's')
  }
  ...
  return (
        <div>
            ...
            {showGitMan ? <GitMan showGitMan={setShowGitMan}></GitMan> : <></>}
        </div>
  )
}
